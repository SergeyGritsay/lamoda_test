...
func (r *ProductPSQL) AddProduct(code int, stockId int, value int64, dynamic bool) error {
	var query string
	...
	if err = t.QueryRow(query, stockId).Scan(&s.IsAvailable); err != nil {
		log.Println(err)
		return err
	}

	if s.IsAvailable {
		query = fmt.Sprintf(`update %s set value = (select value from %s where code = $2) + $1 where code = $2 and stock_id = $3`, productTableName, productTableName)

		_, err = t.Exec(query, value, code, stockId)
	} else {
		switch dynamic {
		case true:
			query = `select s.id from warehouse s
							inner join product g on g.code = $1 and g.stock_id = $2
						where s.id != $2 and s.available limit 1`

			if err = t.QueryRow(query, code, stockId).Scan(&s.ID); err != nil {
				return err
			}

			query = fmt.Sprintf(`update %s set value = (select value from %s where code = $2) + $1 where stock_id = $3`, productTableName, productTableName)

			_, err = t.Exec(query, value, code, s.ID)
